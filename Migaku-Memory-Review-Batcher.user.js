...
(function () {
  ...
  const BOX_RIGHT_OFFSET = 105;
  const DEFAULT_BATCH_SIZE = 10;
  const ROUTE = '/study';
  const POLL_MS = 1000;
  const SELECTOR_COUNTER = '.StudyHeader__reviewsLeft';
  const SELECTOR_CLOSE   = 'button[aria-label="ID:StudyHeader.close"]';
  const LS = {
    BATCH_SIZE: 'migaku_reviews_threshold',
  ...
  function applyHideStyles() {
    ...
    if (hideProgress)
      css += `.UiProgressBar__svg, .UiProgressBar__squiggle, .ProgressBar__container {display:none !important;}\n`;
  ...
  function createBox() {
    ...
    box.innerHTML = `
      ...
      <div class="controls" style="flex-direction:column; align-items:flex-start;">
        ...
        <label title="Hide .-accent-4.-rounded.-solid.UiStatusLabel elements">
